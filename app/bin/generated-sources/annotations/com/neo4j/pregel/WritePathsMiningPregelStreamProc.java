...
public final class WritePathsMiningPregelStreamProc extends BaseProc {
    @Procedure(
    ...
    public Stream<PregelStreamResult> stream(@Name("graphName") String graphName,
        ...
        var specification = new WritePathsMiningPregelStreamSpecification();
        var executor = new ProcedureExecutor<>(specification, executionContext());
        return executor.compute(graphName, configuration);
    }

    @Procedure(
            name = "esilv.pregel.find_paths_with_write.stream.estimate",
            mode = Mode.READ
    )
