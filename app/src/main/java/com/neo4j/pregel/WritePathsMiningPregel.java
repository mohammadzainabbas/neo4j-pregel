...
public class WritePathsMiningPregel implements PregelComputation<WritePathsMiningPregel.WritePathsMiningPregelConfig> {
    ...
    public ArrayList<Long> readOutputResultFromFile(ComputeContext<WritePathsMiningPregelConfig> context, String filePath) {
    ...
    public void compute(ComputeContext<WritePathsMiningPregelConfig> context, Messages messages) {
        ...
        String file_name = context.config().writePath() + File.separator + nodeOriginalId + ".txt";
        ...
        if (superstep == context.config().maxIterations() - 1) {
            addOutputResults(context, file_name);
        }
    }
