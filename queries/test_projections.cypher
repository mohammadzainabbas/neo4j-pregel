...
CREATE CONSTRAINT patent_unique_id IF NOT EXISTS FOR (n:Patent) REQUIRE n.id IS UNIQUE;

LOAD CSV FROM 'file:///Users/mohammadzainabbas/Downloads/cit-Patents.txt' AS line FIELDTERMINATOR '\t'
WITH line
LIMIT 10000
MERGE (referenced:Patent {id: toInteger(line[0])})
MERGE (citing:Patent {id: toInteger(line[1])})
MERGE (citing)-[:CITES]->(referenced);
...
WITH -1 as identifier
CALL esilv.pregel.find_paths.stream("us_patents", {maxIterations: 5, identifier: identifier})
YIELD nodeId, values
UNWIND values.paths AS paths
WITH collect(paths) as path, identifier
CALL esilv.proc.find_signatures(path, identifier) YIELD signature, count
